...
export default function AddColor() {
    ...
    if(!updateIdState){
      ...
      .then((result) => {
        if(result.data.status == true){
          ...
          navigate('/dashboard/color/view-color');
        } else {
      ...
      axios.put(`http://localhost:5000/api/admin/color/update/${ updateId }`, data)
      .then((result) => {
        if(result.data.status == true){
          toast.success(result.data.message);
          navigate('/dashboard/color/view-color');
  ...
  return (
    <>
 <Breadcrumb path={"Color"} link={"/colors/add-color"} path2={"Add"} slash={"/"} />
