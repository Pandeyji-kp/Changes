...
export default function AddSubCategory() {
  ...
  let [subcategoryDatails, setSubCategoryDatails] = useState('');
  useEffect(() => {
    $(".dropify").dropify({
      messages: {
  ...
  const onSubmit = (data) => {
    ...
    if (!updateIdState) {
      axios.post('http://localhost:5000/api/admin/sub-categories/add', toFormData(data))
        .then((result) => {
          if (result.data.status == true) {
            ...
            navigate('/dashboard/subcategory/view-subcategory');
          } else {
      ...
      axios.put(`http://localhost:5000/api/admin/sub-categories/update/${updateId}`, data)
        .then((result) => {
          if (result.data.status == true) {
            toast.success(result.data.message);
            navigate('/dashboard/subcategory/view-subcategory');
  ...
  useEffect(() => {
    if (updateId == undefined) {
      ...
      axios.post(`http://localhost:5000/api/admin/sub-categories/details/${updateId}`)
        .then((result) => {
          if (result.data.status == true) {
            setSubCategoryDatails(result.data.data);
          } else {
            toast.error(result.data.message);
          }
                ...
                <div className="w-2/3">
                  ...
                  <div className="mb-5">
                    ...
                    <input
                      ...
                      placeholder="Category Name" defaultValue={subcategoryDatails.name}
                    />
                  ...
                  <div className="mb-5">
                    ...
                    <input
                      ...
                      id="categoryName" defaultValue={subcategoryDatails.order}
                      className="text-[19px] border-2 shadow-sm border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full py-2.5 px-3"
