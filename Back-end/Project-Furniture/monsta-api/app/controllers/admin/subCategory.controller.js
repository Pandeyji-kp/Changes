...
exports.parentCategories = async (request, response) => {
    ...
    if (request.body.status != '' && request.body.status != undefined) {
        addCondition.status = request.body.status;
    }
...
exports.index = async (request, response) => {
    ...
    await subCategoryModal.find(condition)
        .select('name request.body.parent_category_id image status order')
        .sort(
            {
                _id: 'desc'
            }
